...
subtract        :   SUBTRACT ats+=atomic+ FROM frm=atomic (GIVING giving=identifiers)?;
multiply        :   MULTIPLY at=atomic BY ats+=atomic+ (GIVING giving=identifiers)?;
...
divide          :   DIVIDE at=atomic INTO ats+=atomic+ (GIVING id=identifiers)? (REMAINDER rem=identifiers)?;
...
loopExpression          :   VARYING id=identifiers? (FROM frm=atomic)? (TO to=atomic)? (BY by=atomic)?             #varyingLoopExp
